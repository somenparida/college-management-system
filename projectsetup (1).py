...
def admin_portal():
    ...
    if option1==1:
        ...
        try:
            ...
            command.execute("INSERT INTO student (Name,password,privilage) VALUES(?,?,?)",query)
            x.commit()
        ...
        try:   #if the table is present adding the values
            ...
            command.execute('INSERT INTO teacher (Name,password,privilage,salary) VALUES(?,?,?,?)',query)
            x.commit()
            ...
            query=(teachername,)
            command.execute('SELECT * FROM teacher WHERE Name=?',query)
            ...
            if len(queryresult)==0:
                ...
                command.execute('UPDATE teacher SET salary=? WHERE Name=?',value) # modifiying  the salary of teacher
                x.commit()
            ...
            query=(studentname,)
            command.execute('DELETE FROM student WHERE Name=?',query) # checking if the student is present if present removed
            x.commit()
        ...
        teachername=input("Enter the teacher name:")
        query=(teachername,)
        command.execute('DELETE FROM teacher WHERE Name=?',query) #checking the if the teacher is present if present removed
        x.commit()
        if command.rowcount<1:
    ...
    if option==1:
        ...
        try:
            for i in studentrecords:
                ...
                command.execute("INSERT INTO attendance1 (Name,date,status) VALUES(?,?,?)",queryvalues)
                x.commit()
...
def teacher():
    ...
    command.execute('SELECT * FROM teacher WHERE Name=? AND password=?',query)
    #queryresult=command.fetchall()
...
def studentconsole():
    ...
    if option1==1:
        ...
        command.execute('SELECT*FROM attendance1 WHERE Name=?',query)
        ...
        if timetable1==2:
            try:
                ...
                query=(day,) 
                command.execute('SELECT*FROM timetable WHERE day=?',query) 
                timetablelist=command.fetchall()
                for i in timetablelist:
                    print('Day:',i[0])       
                    print('subject1:',i[1])
...
def student():
    ...
    command.execute('SELECT*FROM student WHERE Name=? AND password=? AND privilage=?',queryvalue)
    studentlist=command.fetchall()
    for i in studentlist:
        if command.rowcount<0:
